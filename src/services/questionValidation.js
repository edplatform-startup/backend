...
const BATCH_VALIDATION_MODEL = 'google/gemini-2.5-pro';
...
export async function validateCourseContent(allGeneratedContent, userId, courseId) {
  ...
  allGeneratedContent.forEach((lesson, lessonIdx) => {
    ...
    if (payload.quiz && Array.isArray(payload.quiz)) {
      payload.quiz.forEach((q, qIdx) => {
        ...
        const needsValidation = q._needsValidation === true || confidence < CONFIDENCE_THRESHOLD;
    ...
    if (payload.practice_problems && Array.isArray(payload.practice_problems)) {
      payload.practice_problems.forEach((p, pIdx) => {
        ...
        const needsValidation = p._needsValidation === true || confidence < CONFIDENCE_THRESHOLD;
  ...
  // 2. Helper function to validate a chunk of items
  const validateChunk = async (chunkItems, chunkIndex, totalChunks) => {
    const promptItems = chunkItems.map(item => {
      if (item.type === 'quiz') {
        return `ID: ${item.id}
Type: Quiz
...
Sample Answer: ${JSON.stringify(item.sample_answer)}`;
    ...
    const systemPrompt = `You are the Chief Academic Officer validating course content.
Validate and REPAIR every item provided.
...
Output JSON:
{
  "results": {
    ...
    "practice_abc_0": { "status": "discard", "reason": "..." }
  }
}

- "valid": keep original (data: null)
- "fixed": provide FULL fixed object
- "discard": mark unsalvageable
Return results for ALL IDs.`;

    const userPrompt = `Validate these ${chunkItems.length} items:\n\n${promptItems}`;

    console.log(`[validateCourseContent] Processing chunk ${chunkIndex + 1}/${totalChunks} (${chunkItems.length} items)`);
    ...
    const { content } = await grokExecutor({
      ...
      responseFormat: { type: 'json_object' },
      requestTimeoutMs: 180000, // 3 minutes per chunk
      reasoning: { enabled: true },
      userId,
    ...
    const raw = coerceModelText(content);
    if (raw.length < 50) {
      console.error(`[validateCourseContent] Chunk ${chunkIndex + 1} extraction failed. Preview: ${JSON.stringify(content).slice(0, 300)}`);
      return {};
    }

    const parsed = parseJsonObject(raw, `course_batch_validation_chunk_${chunkIndex}`);
    return parsed?.results || {};
  };

  // 3. Split into chunks and validate
  const chunks = [];
  for (let i = 0; i < itemsToValidate.length; i += BATCH_CHUNK_SIZE) {
    chunks.push(itemsToValidate.slice(i, i + BATCH_CHUNK_SIZE));
  }

  console.log(`[validateCourseContent] Split into ${chunks.length} chunks of up to ${BATCH_CHUNK_SIZE} items each`);

  // Validate all chunks and merge results
  let allResults = {};
  try {
    for (let i = 0; i < chunks.length; i++) {
      const chunkResults = await validateChunk(chunks[i], i, chunks.length);
      allResults = { ...allResults, ...chunkResults };
    }
  } catch (error) {
    console.error('[validateCourseContent] Chunk validation failed:', error.message);
